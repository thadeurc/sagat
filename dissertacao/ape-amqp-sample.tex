...
\end{lstlisting}
	...
	Para o nosso exemplo optamos por criar a classe \code{SampleProducer} para 
	encapsular a criação dos objetos no \english{broker} e o envio das mensagens. 
	A classe é apresentada na listagem \ref{lst:producer}. O método \code{startProducer}
	...
	transiente e temporária e faz o \english{binding} de ambos. O método \code{publish}, por 
	...
	omitimos as propriedades adicionais (\code{BASIC\_PROPS}) que podem ser utilizadas no 
...
class SampleProducer extends CommonAMQP {
  ...
  def stopProducer = {    
	...
	\code{SampleProducer}. Nessa listagem fazemos a instanciação e a inicialização
	de um \code{producer}, geramos uma sequência com $100$ mensagens e as
...
object SampleProducerApplication  extends Application {
  ...
  producer.stopProducer
	...
	envio. Criamos a classe \code{SampleConsumer} para encapsular o recebimento, a 
	confirmação e tratamento das mensagens, como mostrado na listagem \ref{lst:consumer}.
	O método \code{startConsumer}, assim como o método \code{startProducer} mostrado
	...
	\code{handleDelivery} é o método invocado a cada mensagem recebida. Podemos
	notar que o método, além de receber o corpo da mensagem, ainda recebe algumas informações sobre
	o envio. Os demais métodos são herdados da interface \code{com.rabbitmq.client.Consumer}
	e servem como notificadores para outros eventos, como registro 
	(\code{handleConsumerOk}) ou cancelamento (\code{handleCancelOk}) de um 
	consumidor na fila. Mais detalhes sobre a interface \code{com.rabbitmq.client.Consumer}
