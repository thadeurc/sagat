...
Gostaria de expressar minha enorme gratidão ao Institudo de Matemática e Estatística
...
que sempre me ajudaram a ficar firme e acreditar no meu objetivo. Ricardo Hermann, 
...
The actor model has been seen as an alternative for the conventional concurrent programming based on
locks and condition variables. Actors are computational agents that comunicate
by sending messages and have a mailbox and a behavior. The messages sent to an actor are stored in its
mailbox and are asynchrously processed.
...
In this document we explored the pontential sinergy between a message broker and an implementation of the actor model.
We created a modified version of the actor model implemented in the Akka project that makes use of a AMQP message broker
as the transport engine for messages to remote actors.
