...
O suporte ao modelo de atores pode estar disponível em um linguagem de programação seja 
fazendo parte da estrutura da linguagem, como uma biblioteca embutida em sua distribuição ou 
ainda como uma biblioteca separada da distribuição padrão. Apresentamos a seguir algumas
linguagens de programação baseadas no modelo de atores, e que possuem primitivas que facilitam
a criação de sistemas baseados em atores. Apresentamos também algumas bibliotecas que adicionam
o suporte ao modelo de atores a linguagens mais gerais.
...
de atores, onde o suporte é dado via primitivas na estrutura da própria linguagem.
...
que trafegam entre neles com o uso das palavras chaves \lstinline$input$ e \lstinline$output$. 
...
\lstinline$behavior$ é análoga a palavra chave \lstinline$class$ de Java, e é utilizada para definir
...
\lstinline$interfaces$, \lstinline$behaviors$ seguem a mesma regra. O \lstinline$behavior$
...
\begin{enumerate}
	...
	da ação a lhe ser enviada o \english{token}, o valor recebido será o resultado da
	...
	é análoga a funções de ordem superior, onde funções podem receber outras funções. No caso
...
\begin{itemize}
	...
	\item Scala: Scala Actors \cite{scala-actors}, Akka \cite{akka} and Scalaz Actors \cite{scalaz};
	
	\item Closure: Agents \cite{closure-agents}.	 
\end{itemize}
...
do escopo deste trabalho. Contudo, Karmani e Agha fizeram uma análise comparativa entre alguns 
...
Apresentamos a seguir informações sobre as bibliotecas de atores que foram consideras como opção
...
Erlang. A biblioteca Scala Actors oferece basicamente as funcionalidades já descritas na 
seção \ref{sec:atores-linguagens-erlang}.

Atores foram projetados como objetos baseados em \english{threads} Java e possuem métodos 
como \lstinline$send$, \lstinline$!$, \lstinline$receive$, além de outros métodos como 
\lstinline$act$ e \lstinline$react$. Cada ator possui uma caixa de correio para o recebimento e 
armazenamento temporário das mensagens. O processamento de uma mensagem é feito por um bloco 
\lstinline$receive$ declarado dentro do método \lstinline$act$, que define os padrões a 
serem casados com a mensagem aguardada e as ações associadas a cada padrão. 
...
Atores são executados em um conjunto de \english{threads}\footnote{Idealmente o tamanho do 
conjunto corresponde ao número de núcleos do processador}. O conjunto cresce conforme a demanda. 
...
passe a reagir aos eventos (recebimentos de mensagens). As implicações práticas do uso do 
...
uma mensagem (o uso dos laços tradicionais bloqueariam a \english{thread}); e de o ator
ser responsável por invocar eventuais métodos após o processamento de uma mensagem 
\cite{scala-actors}.	

Além de métodos para envio de mensagem equivalentes às primitivas de Erlang, a biblioteca de atores
...
com os atores de Erlang. Em Scala não é possível a criação de um ator em um nó que não seja o 
local, ou seja, \english{remote spawns} não são possíveis. Atores são acessíveis remotamente 
via \english{proxies}. Para obter uma referência a um ator remoto, um cliente faz uma busca  
em um determinado nó (uma JVM identificada por um par hospedeiro e porta), utilizando 
...
O projeto Akka é composto por um conjunto de módulos escritos em Scala
...
e tolerantes a falhas. Na sua versão $1.0$, suas principais características são: 
...
seguintes opções: JSON \cite{json}, Protobuf \cite{protobuf} e seriação
Java padrão. O transporte das mensagens é feito via TCP/IP, com o auxílio do JBoss 
...
\subsubsection{O projeto Scalaz}
