...
dinâmicas, onde agentes podem ser criados ou destruídos, e reconfigurações como migrações 
...
Mesmo que a interface de um ator não permita o compartilhamento do seu estado, mensagens enviadas poderiam expor um compartilhamento indesejado
de estado com atores colocados. O uso de mensagens imutáveis ou passadas via ``cópia funda'' é uma abordagem mais apropriada para evitar um compartilhamento indesejado de memória entre atores colocados.
...
a eventos onde cada ator guardava um histórico dos seus eventos \cite{greif:1975}. 
O objetivo do estudo era analizar a relação causal entre os eventos. Em $1977$, Baker e Hewitt formalizaram um conjunto de axiomas para computação concorrente \cite{baker:1977}. No mesmo ano, Hewitt 
...
fazendo parte da estrutura da linguagem, ou ainda como uma biblioteca. Apresentamos na sub-seção 
...
e são definidos com o uso da palavra-chave \code{agent}. A troca de
mensagens é feita via canais. Os canais são responsáveis por definir os tipos de dados
que trafegam neles com o uso das palavras-chaves \code{input} e \code{output}. 
...
\code{domain} e permitem que agentes definidos dentro de um domínio compartilhem informações
...
\code{behavior} é semelhante a construção \code{class} de Java, e é utilizada para definir
...
\code{salsa.language.UniversalActor} é análogo a classe \code{java.lang.Object}, e todos os
\english{behaviors} o extendem direta ou indiretamente. A primitiva \code{<-} é utilizada
para enviar uma mensagem a um ator.

A implementação de atores da linguagem utiliza o objeto \code{salsa.language.Actor}
como ator base que extende \code{java.lang.Thread}, criando uma relação onde 
cada ator executa em uma \english{thread}. Cada ator possui uma fila de mensagens onde
as mensagens recebidas ficam armazenadas até que o método \code{run} as retire
...
\begin{enumerate}
	...
	\begin{center}
		\code{gerente <- aprovacao1(500)} \\
		\code{@ gerente2 <- aprovacao2(token)} \\
		\code{@ diretor <- aprovacaoFinal(token);}
	\end{center}
	
	Nesse exemplo, \code{gerente} processa a mensagem \code{aprovacao1(500)} e produz
	um valor que é passado como \code{token} no envio da mensagem para o ator
	\code{gerente2}. A palavra \english{token} é uma palavra-chave e seu valor é associado 
	no contexto do último \english{token} passado. O ator \code{diretor} recebe como argumento
	o resultado da computação do ator \code{gerente2}. Ainda nesse exemplo, utilizamos
	a primitiva \code{@} para indicar que desejamos utilizar a continuação com passagem
	de \english{token};
	...
	\begin{center}
		\code{join(gerente <- aprovacao1(500),} \\
		\code{gerente2 <- aprovacao2(500))} \\
		\code{@ diretor <- aprovacaoFinal(500);}
	\end{center}
	
	Nesse exemplo, os dois atores \code{gerente1} e \code{gerente2} fazem suas aprovações
	em paralelo e, somente quando os dois tiverem dado suas aprovações, o ator \code{diretor}
	irá receber a mensagem de \code{aprovacaoFinal};
	...
	pela palavra-chave \code{currentContinuation}.
...
atribuição única, casamento de padrões e um conjunto de primitivas que inclui \code{spawn}
para criação de atores, \code{send} e \code{!} para o envio de mensagens, \code{receive}
para o recebimento de mensagens e \code{link} para a definição de adjacências entre atores. 
...
Erlang possui o módulo \code{erl\_prim\_loader} que auxilia no carregamento de módulos em
...
comparações entre as várias implementações da semântica de execução e das abstrações. 

Apresentamos a seguir informações sobre duas bibliotecas de atores que foram consideras como opções
...
Java. Elas possuem métodos como \code{send}, \code{!}, \code{receive}, além de 
outros métodos como \code{act} e \code{react}. Cada ator possui uma caixa de correio 
para o recebimento e armazenamento temporário das mensagens. O processamento de uma mensagem 
é feito por um bloco \code{receive}.

No bloco \code{receive} são definidos os padrões a serem casados com as mensagens 
...
É importante ressaltar que o uso do método \code{receive}
...
\code{react} ao invés do método \code{receive} sempre que possível, pois dessa forma 
um ator que não estiver em execução cederá sua \english{thread} para que ela execute outro ator.
Do ponto de vista de funcionalidade, o método \code{receive} é bloqueante e pode devolver 
valores, enquanto que o método \code{react}, além de não devolver valores, faz com que o ator
passe a reagir aos eventos (recebimentos de mensagens). Da perspectiva do programador, uma
implicação prática do uso de \code{react} em vez de \code{receive}
é o uso da estrutura de controle \code{loop} ao invés de laços tradicionais, 
...
específicas. Esses métodos são: \code{!?}, que faz envio síncrono e aguarda uma resposta 
dentro de um tempo limite especificado e \code{!!}, que faz o envio assíncrono da 
...
de Scala, embora também sega as idéias de Erlang. O comportamento dos atores Akka no 
recebimento de mensagens inesperadas (que não casam com nenhum dos padrões especificados em um 
\code{receive}) é diferente dos atores de Erlang e Scala, onde o ator é suspenso. No caso 
...
\code{!!}, semelhante ao método \code{!?} da biblioteca de atores de Scala, no qual o
remetente fica bloqueado aguardando uma resposta durante um tempo limite, e \code{!!!}, que é 
semelhante ao método \code{!!} da biblioteca de atores de Scala, o qual devolve um resultado
futuro ao remetente.
