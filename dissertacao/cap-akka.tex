...
de resposta via método \code{self.reply}. No caso dos envios de mensagem com resposta
ao remetente, a chamada \code{self.reply} é obrigatória. A listagem \ref{lst:method!!}
mostra a assinatura de um dos métodos de envio, o método \code{!!}. Note a presença
do parâmetro implícito \code{sender}, que normalmente não é fornecido pelo chamador
do método.
...
de dois atores pode acontecer via métodos \code{link} e \code{startLink}, definidos em \code{ActorRef}. 
A chamada \code{a1.link(a2)} faz a ligação dos atores \code{a1} e \code{a2}. A chamada
\code{a1.startLink(a2)} também faz a ligação dos atores \code{a1} e \code{a2}, porém
coloca o ator \code{a2} no estado ``iniciado''. As ligações são unidirecionais, ou seja,
o ator alvo da chamada que criou uma ligação (\code{a1}) receberá uma notificação 
em caso de falha em algum um ator ligado a ele (\code{a2}).
...
\begin{enumerate}
	...
	Este ciclo é definido pela classe \code{Supervision.Permanent}.
	...
	chamada ao método \code{stop}). Este ciclo é definido na classe \code{Supervision.Temporary}.
\end{enumerate}
...
classe que define o ator. Os métodos de \english{callback} \code{preRestart} e \code{postRestart} são 
...
\begin{itemize}
	...
	\item A nova instância criada utiliza a mesma \code{ActorRef} da instância em que ocorreu o erro,
	ou seja, tanto a instância nova quanto a anterior possuem o mesmo valor de \code{self}.
...
definimos a estratégia um por um, tratando exceções do tipo \code{java.lang.Exception}, 
...
\begin{itemize}	
	...
	mensagens de resposta vindas do \code{RemoteServerModule}, no caso de envios via \code{!!}
	ou \code{!!!}. A documentação do Akka utiliza para este componente a terminologia
	``cliente remoto'' (\english{remote client}).

	\item \code{RemoteSupport}: Esta classe abstrata é responsável por concentrar as 
...
Na linha $3$ registramos o ator sob o nome \code{hello-service}. O ator passou a 
...
tornando opcional a chamada ao método \code{start} no ator. Note que o fato de um ator ser remoto 
...
Referências para atores remotos são obtidas via método \code{actorFor}. Esse
...
maioria das vezes, uma instância de \code{RemoteActorRef} que representam \english{proxies} 
...
mesmo nó que o ator remoto, uma instância de \code{LocalActorRef} é devolvida. 
...
A biblioteca de atores fornece uma versão alternativa do método \code{actorFor}
...
\code{actorFor}: é necessário que esse servidor esteja em execução antes do método \code{actorFor}
...
instanciação de um componente do Netty para monitorar a \english{socket} associada ao par hospedeiro e porta. 
...
\begin{enumerate}
	...
	No caso de envio assíncrono, o ator destinatário tem seu método \code{!} 
	invocado. Além de receber como parâmetro a mensagem, essa chamada ao método \code{!}
	recebe um segundo parâmetro (que normalmente não é fornecido por ser implicitamente 
	capturado) com uma referência para o ator remetente (\code{sender}) ou com o valor \code{None},
...
Os envios feitos via métodos \code{!!} e \code{!!!} para atores remotos são análogos,
...
é colocada no mapa de resultados futuros uma referência para a instância de \code{CompletableFuture} 
criada pela chamada ao método \code{!!} ou \code{!!!}. A chave para essa instância é o identificador da mensagem.

Um envio feito mediante chamada ao método \code{!!} ou \code{!!!} gera uma mensagem de resposta 
...
no mapa de supervisores e recebe uma mensagem assícrona de notificação enviada localmente.
...
A linguagem declarativa utiliza a palavra-chave \code{message} para definir o formato de uma mensagem. 
No contexto do Protobuf, o termo ``mensagem'' signfica ``tipo de dados estruturado''.
...
com as palavras-chaves \code{required} e \code{optional}.
...
junto à mensagem, foi definido um formato para as \code{RemoteActorRef}s.
...
durante um envio síncrono ao ator (linha $5$). O formato de mensagem \code{AddressProtocol} 
...
Na linha $2$, o atributo \code{uuid} é um identificador único da mensagem.
O formato \code{UuidProtocol} utiliza a biblioteca UUID \cite{uuid},
que é uma implementação de identificadores globais únicos. Podemos notar na linha $5$ um 
atributo booleano que indica se é esperada uma mensagem de resposta. As mensagens enviadas via 
método \code{!} possuem valor \code{oneWay = true}, enquanto que as enviadas via métodos \code{!!}
e \code{!!!} possuem valor \code{oneWay = false}.
...
O módulo \code{Serializable} define um conjunto de feições que podem ser utilizadas para 
trocar a seriação Java por algum dos outros formatos. O módulo \code{Serializer} define um 
objeto chamado de \code{MessageSerializer} que é o responsável pela seriação e 
...
do módulo \code{Serializable} mencionadas acima, será usado o tipo de seriação correspondente
à essa feição. Por exemplo, se a instância utilizada como argumento para um envio de mensagem for combinada com
a feição \code{ScalaJSON}, o conteúdo binário da mensagem estará no formato JSON. Caso a mensagem
