...
seriação de mensagens e no protocolo que foi definido para o envio de mensagens.
        
\section{Atores locais}
\label{sec:atores_locais}
\index{Akka!atores locais}

A definição de atores locais acontece se combinando a feição \code{akka.actor.Actor}, 
e provendo uma implementação para o método \code{receive}, como mostrado na listagem \ref{lst:def-actor}. 
...
pelo arcabouço) é uma instância de \code{akka.actor.ActorRef}. Essas referências
...
define o comportamento do ator pode alterar as definições padrão providas pelo arcabouço e utilizar 
seus métodos para, por exemplo, responder ou encaminhar mensagens recebidas e acessar a referência de 
um ator supervisor. Vale mencionar que, caso de atores locais, \code{self} referencia uma instância de \code{akka.actor.LocalActorRef}.
...
Atores são criados pelo método \code{actorOf} do objeto \code{akka.actor.Actor}
...
O método \code{actorOf} é sobrecarregado para permitir que uma função sem 
...
estado ``iniciado'' e está apto a receber mensagens. Uma vez que o método \code{exit} ou 
\code{stop}\footnote{O método \code{exit} invoca internamente o método 
\code{stop} que é responsável por desligar o ator.} é invocado, o ator passa 
ao estado de ``desligado'' e não executa mais ação alguma.
...
\begin{itemize}
	...
	Definido na classe \code{ExecutorBasedEventDrivenWorkStealingDispatcher}, é
	um despachador para ser usado em atores do mesmo tipo, já que permite que
	atores que não estejam processando mensagens possam ``roubar'' mensagens das filas dos atores
	que estão sobrecarregados, permitindo o balanceamento do processamento das mensagens entre os atores;
	
	\item Despachador quente: 
	É um despachador inspirado no \english{Grand Central Dispacher} do Mac OS X \cite{apple-gcd}. Esse despachador 
	...
	Sua grande vantagem, é a capacidade de agrupar diversos eventos gerados pela
	...
	É o despachador mais simples de todos os dispachadores, já que associa uma \english{thread} para 
	...
	Este despachador é definido na classe \code{ThreadBasedDispatcher}.
\end{itemize}
...
antes de o ator ser inciado via método \code{self.dispatcher}. O Akka permite ainda
que novos dispachadores sejam criados. Por padrão, os despachadores são criados no pacote
\code{akka.dispatch}.
...
fazendo o envio (\english{sender}). A assinatura do método \code{!!} é mostrada na 
...
O ator destinatário pode responder a uma mensagem via método \code{self.reply}. 
...
algumas facilidades em relação a implementação de Java que são necessárias
pelo arcabouço. A feição \code{CompletableFuture} define métodos
...
A implementação do Akka é construída sobre classes do pacote \code{java.util.concurrent}.
...
Envios de mensagens feitos via métodos \code{!!} e \code{!!!} são 
...
Na \code{ActorRef}, um envio de uma mensagem feito via método \code{!!!} é processado 
da seguinte maneira:

\begin{enumerate}
	\item Uma instância de \code{CompletableFuture} é criada dentro do método;
	
	\item Assim como para um envio assíncrono, a mensagem é colocada da fila do ator. Contudo,
	uma referência para a instância criada no passo anterior é colocada junto da mensagem;
	
	\item É retornado para quem invocou o método uma outra cópia da referência para o 
	\code{CompletableFuture} criado no passo $1$;
	
	\item Quando a mensagem terminou de ser processada, seu resultado é atualizado
	na referência que foi enviado junto à mensagem. Como a instância que teve
	o resultado ``completado'' (seja com um valor ou com uma exceção) também
	é referenciada por quem fez a invocação do método, o resultado pode então
	ser utilizado.
\end{enumerate}
...
da computação assíncrona acontece de modo bloqueante dentro do próprio método \code{!!}.
...
boleano \code{true} para indicar se houve envio ou \code{false} caso contrário.
...
\begin{enumerate}
	\item Permantente: Indica que o ator possui um ciclo de 
...
\begin{itemize}
	\item O método \code{preRestart} é invocado na instância onde ocorreu o erro;

	\item O método \code{postRestart} é invocado na nova instância;

	\item A nova instância criada utiliza a mesma \code{ActorRef} do ator onde ocorreu o erro,
...
\begin{itemize}	
	...
	Ademais, oferece também suporte em tempo de execução para a infra-estrutura de atores do lado do 
...
uma cópia da referência da instância do resultado futuro que é retornada é colocada 
...
classe. A listagem \ref{lst:akka-conf-vanilla} mostra a seção reference às configurações do módulo de atores remotos.
...
\code{message}. Protocolos podem contér outros protocolos e tipos primitivos 
de dados, como por exemplo números, textos, valores boleanos e vetores de dados. Ademais, pode-se
