...
entidade conheça o endereço do nó hospedeiro onde está a outra entidade. Nos casos onde as 
entidades estão em nós que não fazem parte de uma rede interna,
a informação da localidade pode não indicar exatamente o nó corrente da entidade, 
...
para um entre portas e entidades. Os protocolos TCP e UDP utilizam para a numeração
...
do nó hospedeiro e da porta e ficam registrados no \code{RemoteServerModule}. 
Vimos também, na seção \ref{sec:atores_locais}, que atores são identificáveis, 
o que permite que eles sejam localizados no \code{RemoteServerModule} 
para que possam receber as mensagens. Ainda que a implementação de atores 
remotos do Akka agrupe os atores, removendo a relação do limite do número de 
atores com o número de portas disponíveis, as entidades \code{RemoteServerModule} 
e \code{RemoteClientModule} ainda assim possuem o conhecimento 
do hospedeiro e da porta no qual se está conectado.
...
junto com a informação do hospedeiro durante a criação de atores remotos.
O nome da máquina virtual é definido durante a inicialização da máquina virtual
via parâmetro \code{-name}. Diversas máquinas virtuais podem estar em
execução em um determinado hospedeiro e são unicamente identificadas
por \code{name@host}. Vale ressaltar que as máquinas virtuais Erlang
...
introduz novas características à comunicação. As entidades passam 
a não estarem mais conectadas diretamente, já que o \english{message broker} passa a ser 
...
O \english{message broker} passa a ser o responsável por abrir uma porta para que
as demais entidades possam se conectar e é responsável por fazer o gerenciamento de
múltiplas conexões nesta porta. 

Tanto a rotulação das entidades por hospedeiro e porta ou por 
nome e hospedeiro deixam de fazer sentido pois são redundantes. 
As entidades passam a ser identificadas somente por seus nomes.
Cada nova entidade que entrar no sistema é responsável por registrar no \english{message broker} uma fila 
...
de transporte remoto do projeto Akka é
...
do \english{message broker} RabbitMQ.

\subsection{Pontes AMQP}
\index{Estrutura!message broker amqp!pontes}
\label{sec:entidades-via-broker-amqp-bridges}

O módulo \code{AMQPBridge} é responsável por fornecer classes que agem como pontes de comunicação
...
entidade servidora. O módulo \code{AMQPBridge} contém as seguintes classes:

\begin{itemize}
	\item \code{AMQPBridge}: É a classe abstrata que define o comportamento básico
	de uma entidade conectada a um \english{message broker} AMQP. O construtor de \code{AMQPBridge}
	...
	como mostra a listagem \ref{lst:amqp-bridge}. A classe possui também um objeto companheiro que
	define métodos para a criação de instâncias das subclasses de \code{AMQPBridge};

	\item \code{MessageHandler}: É a feição que define os métodos a serem
	invocados quando mensagens forem recebidas ou envios feitos pela entidade forem 
	rejeitados pelo \english{message broker}; 
	
	\item \code{ServerAMQPBridge}: É uma das duas subclasses de \code{AMQPBridge} e
	define o comportamento para entidades que atuam como servidoras. 
	Além dos métodos herdados, essa classe ainda define o método \code{setup}. Esse
	método recebe como argumento uma implementação de \code{MessageHandler} e os
	...
	\item \code{ClientAMQPBridge}: É a outra subclasse de \code{AMQPBridge} e
	define o comportamento para entidades que atuam como clientes. Essa
	classe tem em seu construtor um parâmetro adicional em relação à classe \code{ServerAMQPBridge}.
	O parâmetro \code{id} é utilizado para a identificação do cliente. A identificação é necessária 
	para compôr a chave de roteamento que 
	associa a fila que essa ponte irá criar no método \code{setup}, com
...
Definimos também um módulo auxiliar chamado \code{AMQP}. Esse módulo possui algumas 
classes com configurações pré-definidas para os objetos a serem criados no \english{message broker}. 
A enumeração \code{StorageAndConsumptionPolicy} define um 
conjunto de valores para políticas de armazenamento e de consumo. 
A enumeração é composta pela combinação da enumeração \code{QueueConfig} 
e \code{ExchangeConfig}. Essas duas enumerações definem as configurações específicas 
...
\begin{enumerate}
	\item \code{DURABLE}: Define uma configuração onde os objetos criados são duráveis,
	ou seja, continuam a existir caso o \english{message broker} seja reiniciado;
	
	\item \code{TRANSIENT}: Define uma configuração onde os objetos criados são transientes,
	ou seja, deixam de existir caso o \english{message broker} ser reiniciado;
	
	\item \code{EXCLUSIVE\_AUTODELETE}: Define uma configuração onde os objetos estão
	...
	a um único consumidor. O consumidor fica atrelado ao canal por onde a criação da 
	fila ocorreu.
\end{enumerate}
...
O módulo \code{ConnectionPoolSupervisor} é responsável por encapsular os detalhes 
...
A classe \code{SupervisedConnectionWrapper} define uma conexão supervisionada, expondo 
...
executar efetivamente as ações, já que a classe \code{SupervisedConnectionWrapper}
...
As principais classes do módulo \code{ConnectionPoolSupervisor} são:

\begin{itemize}
	\item \code{BridgeConsumer}: É uma implementação de um consumidor padrão
	de mensagens. Essa implementação estende a classe \code{DefaultConsumer}
	do RabbitMQ, e utiliza a implementação do \code{MessageHandler} para a
	notificação de eventos como mensagens recebidas ou mensagens rejeitadas e retornadas;
	
	\item \code{ChannelActor}: É a feição que define o comportamento inicial
	de um ator responsável por um canal genérico;
	
	\item \code{ReadChannelActor}: É a classe que define o comportamento inicial
	de um ator responsável por um canal de leitura, utilizado para o recebimento de
	mensagens;

	\item \code{WriteChannelActor}: É a classe que define o comportamento inicial
	de um ator responsável por um canal de escrita, utilizado para o envio de mensagens;

	\item \code{ConnectionActor}: É a classe que define o comportamento inicial
	...
	\item \code{AMQPSupervisor}: É a feição que define o comportamento
	...
	\item \code{AMQPConnectionFactory}: É a classe raiz da hierarquia
	de supervisão. Essa classe herda de \code{AMQPSupervisor} e
	...
	\item \code{ConnectionSharePolicy}: É a enumeração que define a política
...
para definir as mensagens que cada ator aceita em sua função \code{receive},
como por exemplo as mensagens \code{RemoteServerSetup} e \code{ServerSetupInfo} 
utilizadas na listagem \ref{lst:amqp-server-bridge}. Um outro exemplo são as mensagens  
\code{RemoteClientSetup} e \code{ClientSetupInfo} utilizadas 
na listagem \ref{lst:amqp-client-bridge}. A figura \ref{fig:amqpbridge-classes-basic} mostra
como os módulos \code{AMQP}, \code{AMQPBrigde} e \code{ConnectionPoolSupervisor} estão
relacionados.
...
é iniciado nas pontes AMQP. O método \code{setup} das classes \code{ServerAMQPBridge} e
\code{ClientAMQPBridge} serve como ponto de entrada para a criação e configuração
dos objetos. A criação de uma ponte AMQP é feita via métodos definidos 
no objeto \code{AMQPBridge} e depende da criação de uma conexão supervisionada. 

A conexão supervisionada é criada pelo objeto \code{AMQPConnectionFactory}. 
...
\begin{enumerate}
	\item Uma instância de \code{ConnectionActor} é criada e inicializada;
	
	\item O ator é ligado (\code{link}) ao ator supervisor\footnote{A classe 
	\code{AMQPConnectionFactory} é o ator supervisor de todos os atores de conexão.};
	
	\item A mensagem \code{Connect} é enviada sincronamente para o ator;
	
	\item A mensagem \code{Connect} é processada pelo ator. Seu processamento leva
...
\begin{enumerate}
	\item Uma instância de \code{ReadChannelActor} é criada e inicializada;
	
	\item O ator é ligado (\code{link}) ao ator supervisor, que neste caso
	é o ator de conexão recém criado;
	
	\item A mensagem \code{StartReadChannel} é enviada sincronamente para o ator;
	
	\item A mensagem \code{StartReadChannel} é processada pelo ator. Seu processamento leva
	ao envio síncrono da mensagem \code{ReadChannelRequest} para o ator de conexão que foi
	definido como supervisor;
	
	\item O ator de conexão solicita um novo canal à sua conexão de leitura;

	\item O novo canal é enviado como resposta para \code{ReadChannelRequest}.
\end{enumerate}
...
São utilizadas as mensagens \code{StartWriteChannel} e \code{WriteChannelRequest}, respectivamente. 
...
criada tem seu método \code{setup} executado. A execução do método \code{setup}
tem diferentes passos para cada tipo de ponte. A execução do método \code{setup} da classe 
\code{ServerAMQPBridge} é ilustrada na figura \ref{fig:server-amqp-bridge-setup} e 
acontece em oito passos:

\begin{enumerate}
	\item O método \code{setup} da classe \code{SupervisedConnection}
	é invocado;
	
	\item A mensagem \code{RemoteServerSetup} é enviada para o canal de escrita
	com as configurações dos objetos a serem criados;
	
	\item O processamento da mensagem \code{RemoteServerSetup} leva a criação
	de uma \english{exchange} direta com a configuração de durabilidade solicitada;
	
	\item A implementação de \code{MessageHandler} informada é utilizada na
	associação do tratador de mensagens retornadas (\english{return listener});
	
	\item A mensagem \code{RemoteServerSetup} é enviada para o canal de leitura
	com as configurações dos objetos a serem criados;
...
A execução do método \code{setup} da classe \code{ClientAMQPBridge} 
...
\begin{enumerate}
	...
	\item A mensagem \code{RemoteClientSetup} é enviada para o canal de escrita
	com as configurações dos objetos a serem criados;
		
	\item A implementação de \code{MessageHandler} informada é utilizada na
	associação do tratador de mensagens retornadas;
	
	\item A mensagem \code{RemoteClientSetup} é enviada para o canal de leitura
	com as configurações dos objetos a serem criados;
	
	\item O processamento da mensagem \code{RemoteClientSetup} leva a criação
...
ator via método \code{Actor.registry.unregister(actor)}. Nossa hierarquia de
atores, por sua vez, pode ter sua execução interrompida via método 
\code{AMQPConnectionFactory.shutdownAll}.

Mostramos na figura \ref{fig:bridges-broker-all-together} como ficaria, da perspectiva
do \english{message broker}, a criação
de uma ponte servidora de nome \code{node1} com duas pontes clientes associadas
...
para envio de mensagens para todas as filas criadas para o nó \code{node1}. Podemos
notar também como os padrões definidos para os nomes na classe \code{AMQPBrigde} 
...
do método \code{sendMessageToServer} da classe \code{ClientAMQPBridge}. A
execução do método é ilustrada na figura \ref{fig:bridge-send-message-flow} e acontece em
quatro passos:

\begin{enumerate}
	\item O método \code{sendMessageToServer} foi invocado por alguma classe que deseja
	enviar uma mensagem;
	
	\item O método \code{publishTo} da classe \code{SupervisedConnectionWrapper}
	...
	\item O método \code{publishTo} faz um envio assíncrono da mensagem 
	\code{BasicPublish} ao ator responsável pelo canal de escrita. A mensagem
	\code{BasicPublish} possui informações sobre detalhes do envio a ser executado
	como por exemplo, se o envio deve ser mandatório e o consumo imediato;	
	
	\item O método \code{basicPublish} é invocado na classe \code{Channel} do 
	RabbitMQ como resultado do processamento da mensagem \code{BasicPublish}. Os 
	argumentos utilizados para execução do método são os que foram passados junto à
	mensagem \code{BasicPublish}.
\end{enumerate}
...
Optamos por fazer envios mandatórios porém com consumo não imediato. Isso significa que
...
\code{mandatory} e \code{immediate} são respectivamente \code{true} e 
\code{false}. Caso ocorra um envio massivo de mensagens, o consumo imediato das mensagens 
...
\code{handleRejected} do \code{MessageHandler} é invocado para que alguma ação 
seja tomada com a mensagem rejeitada. Mensagens podem ser retornadas por basicamente dois motivos:
...
para a \english{exchange} relacionada a \code{ClientAMQPBridge}. 
O identificador do remetente 
(atributo \code{id} da listagem \ref{lst:amqp-client-bridge}) deve ser enviado
como parte da mensagem e esse envio é de responsabilidade do criador da mensagem. 
Essa identificação será utilizada como argumento do método \code{sendMessageTo} 
...
\begin{enumerate}
	...
	\item O método \code{handleDelivery} da classe \code{BridgeConsumer}
	é invocado;
	
	\item O método \code{handleReceived} da implementação de \code{MessageHandler}
	(definida durante a configuração da ponte servidora) é executado pelo \code{BridgeConsumer}
...
Devemos recordar que a execução do método \code{handleReceived}
deve retornar um valor booleano. Esse valor indica se o recebimento deve ser confirmado  
