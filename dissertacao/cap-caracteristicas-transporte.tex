...
conjunto de valores que definem políticas de armazenamento e de consumo. A enumeração é composta pela 
combinação de outras duas enumerações: \lstinline$QueueConfig$ e \lstinline$ExchangeConfig$. 
Essas duas enumerações definem as configurações específicas e detalhadas para 
filas e \english{exchanges}. A combinação dos seus valores dão semântica às 
políticas de configuração. Por exemplo, a configuração \lstinline$EXCLUSIVE_PERSISTENT$ 
presente em \lstinline$StorageAndConsumptionPolicy$ é uma combinação de 
\lstinline$ExchangeConfig.exchangeDurable$ e \lstinline$QueueConfig.exclusiveQueueDurable$, 
e contém informações suficientes para a criação de uma \english{exchange} durável e uma fila 
também durável, com um único consumidor.
...
duas vezes. Um outro passo que também é executado, porém não é mostrado na figura
\ref{fig:new-connection-actor}, é o envio de uma mensagem para todos os atores que estejam
sob supervisão do ator de conexão para que solicitem novos canais. Esse passo só
faz sentido quando uma conexão foi desconectada e o ator foi reiniciado pelo supervisor.
...
do ator responsável pelo canal de leitura são mostrado na figura \ref{fig:new-channel-actor}.
...
Vale destacar que, segundo a específicação do padrão AMQP \cite{amqp}, as implementações 
