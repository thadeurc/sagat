...
\begin{enumerate}
	\item \code{DURABLE}: Define uma configuração no qual os objetos criados são duráveis,
	ou seja, continuam a existir caso o \english{message broker} seja reiniciado.
	
	\item \code{TRANSIENT}: Define uma configuração no qual os objetos criados são transientes,
	ou seja, deixam de existir caso o \english{message broker} ser reiniciado.
	
	\item \code{EXCLUSIVE\_AUTODELETE}: Define uma configuração no qual os objetos estão
...
\begin{enumerate}
	...
	\item a mensagem \code{StartReadChannel} é processada pelo ator (seu processamento faz
	um envio síncrono da mensagem \code{ReadChannelRequest} para o ator de conexão que foi
