...
\index{AMQP!Padrão}
	...
	neutro, eficiente e possui suporte para múltiplos canais. 
	A especificação \cite{amqp-spec} apresenta o protocolo dividido em três camadas,
...
\section{A camada de modelo}		
	...
	A camada de modelo é a responsável pela definição do conjunto de comandos
	...
	\begin{itemize}
		...
		roteada para a fila; tópicos \english{topic}, onde o valor da chave de roteamento 
		não precisa ser exatamente igual às entradas da tabela de \english{bindings}, 
		bastando possuir o mesmo padrão para ser roteada para a fila. 
		Os caracteres coringa suportados são *, que indica
		uma única palavra e \#, que indica zero ou muitas palavras. A chave de roteamento
		deve ser formada por palavras e pontos. Por exemplo, o padrão *.stock.\# casa
		com usd.stock e eur.stock.db, nas não com stock.nasdaq; e \english{fanout},
		onde o roteamento acontece para todas as filas associadas a \english{exchange},
		indepententemente da chave de roteamento.
...
\section{A camada de sessão}		
	...
	A camada de sessão age como uma intermediária entre a camada de modelo
	...
	às interações entre aplicações cliente e a aplicação servidora.
...
\section{Implementações}	
	...
	aberto com uma distribuição em Java e uma em C++. Possui bibliotecas para aplicações 
	cliente em diversas linguagens como por exemplo, Java, C++, Ruby, Python e C\#.Net. 
	A implementação da biblioteca cliente para Java é compatível com o a versão $1.1$ 
	do padrão Java \english{Message Service} \cite{jms}. A implementação está 
	disponível sob a licença Apache $2.0$ \cite{apache-lic}.
	...
	porém é implementado na linguagem Erlang. Possui bibliotecas
	...
	Optamos por utilizar a biblioteca cliente do projeto RabbitMQ. Tomamos como base para
	...
	\begin{itemize}
		...
		de sistemas distribuiídos e concorrentes.
	...
	uma aplicação de exemplo escrita na linguagem Scala, onde uma das aplicações faz o
	papel de um produtor de mensagens e a outra o papel de consumidor.
	...
	Na implementação, as conexões tem como papel principal estabelecer a comunicação da
	aplicação cliente com o \english{broker}. Conexões podem ser vistas como sendo
	a implementação de uma parte considerável da camada de transporte do modelo AMQP.
	...
	dentre outros métodos, o método \lstinline$createChannel$. Esse método devolve
	...
	\begin{figure}[hbtp]
	  ...
	  \caption{Relacionamento entre componentes das camadas de transporte e sessão.}
...
trait CommonAMQP {
  ...
  def connect: Connection = {
	...
	seguida itera na sequência de mensagens fazendo o envio. Para este exemplo em particular, 
...
class SampleProducer extends CommonAMQP {
  ...
  def stopProducer = {    
	...
	\lstinline$SampleProducer$. Nessa listagem fazemos a instanciação e inicialização
...
object SampleProducerApplication  extends Application {
  ...
  producer.stopProducer
	...
	notar que o método, além de receber o corpo da mensagem, ainda recebe informações sobre
	o envio. Os demais métodos são herdados da interface \lstinline$com.rabbitmq.client.Consumer$
	e servem como notificadores para eventos outros eventos, como registro 
...
object SampleConsumerApplication extends Application {
  ...
  consumer.startConsumer
	...
	Para finalizar esta seção, é importante destacar que a implementação de canais 
	...
	Aplicação produtora passa a ter autonomia de fazer fazer o tratamento apropriado.
