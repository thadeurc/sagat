...
a mensagem possam estar disponíveis nas redes das empresas, 
incentivando o desenvolvimento de aplicações interoperáveis baseadas em troca 
de mensagens. 

AMQP é um protocolo binário e assíncrono, com suporte a múltiplos canais. 
A especificação apresenta o protocolo dividido em três camadas, como mostrado 
na figura \ref{fig:protocol_overview}.	
...
Na seção \ref{sec:amqp-modelo} descrevemos a camada de modelo, que é a camada em que 
...
A camada de modelo é a camada em que está definido o conjunto de comandos
...
sobre a qualidade do serviço, oferecer um mapeamento natural entre os 
comandos do protocolo e as bibliotecas do nível de aplicação e proporcionar clareza
em tal mapeamento, de modo que cada comando seja responsável por uma única ação. 
Os componentes desta camada são: 
...
\begin{itemize}
	...
	Na criação de uma fila, várias propriedades podem ser especificadas: a fila pode ser 
	...
	Há vários tipos de \english{exchanges}, porém neste trabalho utilizamos apenas
	\english{exchanges} diretas (\english{direct}). Em tais \english{exchanges}, 
	o valor da chave de roteamento, que é parte do cabeçalho da mensagem, deve ser exatamente igual 
...
Há uma analogia entre o modelo AMQP e sistemas de \english{email}:
\begin{enumerate}	
...
\begin{itemize}
	...
	seja capaz de garantir a execução do comando exatamente uma vez. Utiliza-se um esquema
	de numeração sequencial. A noção de identificação permite a correlação de comandos
	e a devolução de resultados assíncronos. O identificador do comando é disponibilizado
	para a camada de modelo e, quando um resultado de um comando é devolvido, o
	...
	\item Notificação de comandos completados: Diferentemente do conceito de confirmação,
	...
	se recuperar no caso de falhas na rede, a sessão deve ser capaz de reenviar um comando
	cujo recebimento pelo outro par é duvidoso. A camada de sessão provê as
...
janelamento (\english{framing}).  A lista dos requisitos dessa camada 
...
possuir assincronia e neutralidade em relação à linguagens de 
programação.
...
possui bibliotecas para aplicações clientes em diversas linguagens, como 
Java, C++, Ruby, Python e C\#.  A implementação da biblioteca cliente para Java é 
compatível com a versão $1.1$ do padrão Java \english{Message Service} \cite{jms}.

O projeto ZeroMQ é uma implementação de código aberto feita C++, com
bibliotecas disponibilizadas para aplicações clientes em mais de vinte linguagens,
...
outras bibliotecas nativas específicas para cada sistema operacional. 
...
Ele possui bibliotecas para aplicações clientes em diversas linguagens como Java, Erlang e Python. 
...
\begin{itemize}
	...
	\item Possuir código aberto e sua biblioteca para clientes Java ser independente de plataforma.
...
\lstinline{com.rabbitmq.client.Connection}, que pode ser obtida pela classe
\lstinline{com.rabbitmq.client.ConnectionFactory}, via método \lstinline{newConnection}.
É na classe \lstinline{ConnectionFactory} que informamos os valores dos parâmetros necessários
...
uma instância de \lstinline{com.rabbitmq.client.Channel}, na qual
podemos executar os comandos definidos na camada de sessão (listagem \ref{lst:channels}). 
Tais comandos incluem os comandos para criação de \english{exchanges} (\lstinline{exchangeDecla-}\lstinline{re}),
de filas (\lstinline{queueDeclare}), de \english{bindings} (\lstinline{queueBind}) e
os comandos para envio (\lstinline{basicPublish}) e recebimento de mensagens (\lstinline{basicConsume}).

\begin{lstlisting}[caption={Configuração e uso de uma ConnectionFactory.}, label={lst:conn-factory}]
...
\begin{lstlisting}[caption={Criação e uso de canais.}, label={lst:channels}]
...
\lstinline{handleDelivery} (listagem \ref{lst:sample-consumer}). A figura \ref{fig:rabbitmq-colaboracao} mostra como 
uma fábrica de conexões, uma conexão, um canal e um consumidor estão relacionados. 
		
Um detalhe sobre a biblioteca para clientes Java do RabbitMQ é a verificação
da existência de um objeto antes da criação efetiva do objeto. Por exemplo, se tivéssemos
...
seria lançada e o canal utilizado para executar o comando de criação seria fechado. 
Nesse caso, o procedimento correto é remover $F_1$, seja via código ou via alguma aplicação 
administrativa do RabbitMQ, para depois fazer a criação.

\begin{lstlisting}[caption={Registro de um consumidor.}, label={lst:sample-consumer}]		
...
...
	...
	@Override
	def handleDelivery(consumerTag: String, envelope: Envelope, properties: BasicProperties, message: Array[Byte]): Unit = {
...
Canais permitem ainda que sejam registrados tratadores de erros (\lstinline{setReturnListener}), 
para lidar com casos como o de uma mensagem que não possa ser enviada ou entregue
ao seu destinatário. Em situações como essa, o corpo da mensagem e algumas
...
\cite{rabbit-channels} não é segura para acesso concorrente (\english{thread safe}).
Assim sendo, a aplicação tem a responsabilidade de evitar que diferentes \english{threads}
façam uso concorrente de um canal.
