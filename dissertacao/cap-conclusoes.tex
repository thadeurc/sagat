...
inserções ou atualizações, que são executadas na instância principal (\english{master}),
são replicadas para uma ou mais instâncias secundárias (\english{slaves}). Esse módulo utiliza uma combinação
de tabelas transacionais (InnoDB) e mensagens no formato Protobuf \ref{drizzle-replication-slave}. 
O uso das tabelas transacionais tem como característica a criação de um arquivo para armazenamento
...
Quando o módulo de replicação RabbitMQ Integration está ativado, uma aplicação Java associada a
instância principal coleta as informações do arquivo onde as transações foram armazenadas e faz
o envio das mensagens com as transações. Aplicações Java associadas às instâncias secundárias
...
informada durante a ativação do módulo RabbitMQ Integration. As informações para a criação das filas, como
nome, e da instância do Drizzle para onde as transações serão replicadas, são informadas em um arquivo de propriedades.
