...
e entrega das mensagens, concluímos que existe uma vantagem relativa da nossa implemetação
para transporte de mensagens entre atores remotos com padrão AMQP em relação a implementação original do
Akka feita com o Netty. Tal vantagem deve-se ao fato do Netty também possuir tarefas semelhantes, como recebimento e entrega
das mensagens, e de este processamento acontecer nos mesmos nós onde estão em execução as classes que fazem
o processamento das mensagens, como os atores. Conforme a frequência das mensagens foi aumentando e o sistema
foi chegando ao seu ponto de saturação, houve uma redução considerável na diferença de tempo (figura \ref{fig:grafico-er}).
...
Protobuf. O uso das tabelas transacionais tem como 
...
\subsection{Camada de comunicação entre agentes remotos com RabbitMQ}

Arlekar e outros \cite{ctep} apresentam uma proposta de um arcabouço voltado para simulações baseadas
no modelo de agentes. Ainda que nosso trabalho tenha sido voltado ao modelo de atores,
observamos semelhança na definição da camada de comunicação entre agentes remotos utilizada nesse arcabouço.

Esse arcabouço prove a infraestrutura necessária para a criação de agentes, a troca de mensagens entre agentes e
o armazenamento e a exibição das informações geradas no sistema. O arcabouço possui diversos componentes, contudo,
destacamos apenas os três cujo identificamos semelhança com nosso trabalho:

\begin{itemize}
	\item \english{Care Take Agent} (CTA): É um componente análogo ao \lstinline{RemoteSupport} existente 
	na implementação de atores remotos do Akka. Um CTA possui como algumas de suas responsabilidades, 
	criar agentes e controlar a troca de mensagens entre agentes residentes em diferentes CTAs.

	\item Agente: É análogo a um ator. Um agente representa uma entidade que faz parte da simulação como, por exemplo,
	uma pessoa.

	\item Camada de comunicação: É análoga a implementação da camada de transporte do Akka. A camada de comunicação
...
do CTA proprietário da fila e por um nome. É importante comentar que cada CTA é executado em sua própria JVM.
As mensagens que são recebidas em um CTA são repassadas a um ou vários dos seus agentes. O CTA age também como 
\english{proxy} no envio de uma mensagem. Agentes enviam mensagens a outros agentes utilizando o CTA no qual 
reside como \english{proxy} durante o envio.
