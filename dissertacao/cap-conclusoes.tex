...
\begin{enumerate}
	...
	nos atores clientes. Assim, uma nova mensagem só é enviada uma vez que a mensagem anterior
...
quantidade de atores clientes utilizada no teste e seja $r$ o número de repetições; cada ator cliente 
...
Durante a execução dos testes, mesmo diante de um cenário com alta concorrência onde vários 
...
rede é um dos principais fatores de impacto nos resultados. A tabela \ref{tab:medidas-lentas} apresenta 
...
a $26.74\%$, pouco menos que $9$ segundos para completar todos os envios/recebimentos. 

O gráfico da figura \ref{fig:grafico-er-lento} mostra que, com o aumento de atores clientes, o aumento da 
quantidade de envios/recebimentos feitos com nossa implementação não apresentou a mesma melhora de desempenho 
...
notamos que o aumento no número de envios/recebimentos foi mais acentuado no início (de $1$ a $8$ atores clientes) 
...
\hline 
\end{tabular}
\caption{Medidas de tempo do Trading System com atores remotos em rede wireless.}
...
\begin{figure}[!h]
		...
		\includegraphics[scale=.70]{grafico-er-lento} 
 		\caption{Comparação da quantidade de envios/recebimentos na rede wireless.}
...
remotos do Akka para notificar tando um \lstinline$RemoteClient$ que o \lstinline$RemoteServer$ correspondente
não está mais conectado, quando o contrário.

O fato de utilizarmos filas para fazer o armazenamento intermediário das mensagens 
(ainda que, na maior parte do tempo, por um período breve), permite que um \lstinline$RemoteServer$ 
ou \lstinline$RemoteClient$ possam receber mensagens mesmo após sua execução ter sido interrompida. 
...
da mensagem pode, eventualmente, ser diferente do comportamento que o ator possuia no momento do envio.
...
afirmar que o tempo extra pode ser até considerado como despresível. Nossa conclusão é de que as duas 
...
\begin{itemize}
	...
	parte importante da infra-estrutura de um sistema distribuído. O RabbitMQ, por exemplo, possui um
	...
	nossa implementação não fica atrelhada a este detalhe. Os endereços IPs dos nós não possuem relação 
	com os nomes utilizados para registrar ou localizar um ator remoto. Exiete ainda uma outra vantagem que é
	a redução do número de portas que devem ser desboqueadas em um \english{firewall}. Não é necessário
	desbloquear as portas onde cada \lstinline$RemoteServer$ estará disponível. A única porta que precisa estar
	desloqueada para aceitar conexões, é a porta que o \english{message broker} disponibilizou
...
falar de melhorias na parte de exceptions, colocar no akka 2.0 como sip. Cloud, explorar mais o protocolo (?).
