...
\english{deadlocks} e condições de corrida em nossa implementação. Naturalmente, pelo fato de as 
mensagens serem trafegadas duas vezes a mais na rede, a latência da rede é um dos principais fatores que impacta 
os resultados. A tabela \ref{tab:medidas-lentas} mostra as medidas que foram feitas 
na rede com maior latência. Utilizamos a mesma configuração descrita anteriormente, porém removemos os cabos de rede para que os computadores pudessem ser conectados via \english{wireless}. Podemos notar pelos dados da tabela \ref{tab:medidas-lentas}, 
que a diferença de tempo chega, no pior caso onde temos $4$ atores clientes, a pouco mais de $2.5$ vezes o tempo gasto
pela implementação com Netty. Quando observamos os percentis, podemos notar que a 
diferença de tempo das execuções com AMQP que estão abaixo da mediana, foram aproximadamente três vezes mais lentas em 
relação às execuções feitas com Netty ($209.28$\% e  $197.17$\%). Quando comparamos os 
percentis acima da mediana, observamos uma redução razoável na diferença ($179.11$\% e  $145.07$\%). Assim como na 
tabela \ref{tab:medidas}, conforme a quantidade de atores cliente cresce, a diferença de tempo cai chegando
a $26.74\%$, o que significa pouco menos de $9$ segundos para completar todos os envios/recebimentos. O gráfico
da figura \ref{fig:grafico-er-lento} mostra que, com o aumento de atores clientes, o aumento da quantidade de 
envios/recebimentos feitos com nossa implementação não teve o mesmo desempenho que a implementação com o JBoss Netty.
Uma melhora mais significativa passa a acontecer após a quantidade de clientes passar de $10$. Observando o gráfico 
da figura \ref{fig:grafico-er}, podemos notar que o aumento no número de envios/recebimentos foi mais acentuado 
no início (de $1$ a $8$ atores cliente) do que no final. 

%% justificar o motivo de ter acontecido isso
