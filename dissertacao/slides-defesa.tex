...
%% duvida: colocar estrutura dos atores para setup?!?!?!
	\only<1>{
 		Caracter\'isticas:
		\begin{itemize}
			...
			\item Pontes podem criar objetos dur\'aveis, transientes ou exclusivos com auto remo\c{c}\~ao
		\end{itemize}
	}
	\only<2>{
 		\begin{figure}[hbtp]
...
\frame{\frametitle{Novos componentes}
	\only<1>{ 		
  		\begin{figure}[hbtp]
			\includegraphics[scale=.30]{figuras/remote-actor-amqp-colaboracao.pdf}
		\end{figure}
 	}	
}
