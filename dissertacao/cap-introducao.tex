...
\begin{itemize}
	\item Scala possui certa semelhança com Java  C\#, de modo que tanto programas
	...
	\code{object} ao invés de \code{class}. \english{Singleton objects} que
...
\begin{itemize}  
  \item Um protótipo de atores remotos escrito em Scala e que utiliza um \english{message broker} 
  AMQP como mecanismo de transporte de mensagens;

  \item Para dar suporte a nosso protótipo, criamos também uma camada de abstração para acessar
  a biblioteca de conexão do \english{message broker} AMQP. Essa camada foi implementada com atores e
...
\ref{cap:amqp}, apresentamos as camadas do protocolo AMQP, além de citarmos algumas de suas 
implementações. Ainda nesse capítulo, apresentamos algumas das principais classes  da biblioteca 
para clientes Java da implementação RabbitMQ que foi utilizada neste trabalho. 
No capítulo \ref{cap:estrutura-troca-mensagens-amqp}, apresentamos a camada de abstração
criada para prover acesso concorrente seguro as classes de comunicação da biblioteca para clientes
Java do RabbitMQ. No capítulo \ref{cap:atores_remotos_amqp}, apresentamos nossa implementação 
de atores remotos que utiliza a camada de abstração descrita no capítulo \ref{cap:estrutura-troca-mensagens-amqp}.

Finalmente, no Capítulo \ref{cap:conclusoes}, discutimos as conclusões obtidas neste
trabalho. Apresentamos também uma comparação de desempenho entre a implementação 
original de atores remotos do projeto Akka com o protótipo desenvolvido neste trabalho. 
