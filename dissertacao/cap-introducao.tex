...
Sistemas de \english{middleware} orientados a mensagem
...
em \english{remote procedure call} (RPC). A diferença entre cliente e servidor é conceitual 
e somente pode ser definida por humanos que conheçam a semântica das trocas de mensagens. 
...
Filas transacionais são uma abstração usada para garantir que toda mensagem recebida pelo 
MOM seja salva, de modo persistente, e que a remoção da mensagem ocorra somente após a 
confirmação do recebimento pelo destinatário. Em caso de queda do sistema, quando ele for 
reiniciado, ocorrerá a entrega das mensagens que foram salvas de modo persistente e cujo 
recebimento não foi confirmado. A retirada de uma mensagem de uma fila 
transacional ocorre como parte de uma transação atômica que pode inclui também outras
operações, como envios de mensagens e atualizações em bancos de dados, bem 
como outras retiradas de mensagens.
...
\english{Message brokers} são descendentes diretos de MOMs que endereçam essas limitações. 
Eles agem como intermediários e provêm maior flexibilidade para roteamento e filtragem, 
além de permitirem que se adicione lógica de negócios para o processamento
...
A especificação AMQP (\english{Advanced Message Queuing Protocol}) é uma proposta recente 
...
de programas está relacionado com o modo com que programas são escritos. Para usufruir ganhos 
...
blocos de código marcados como sincronizados. Essa abordagem não permite a componibilidade 
de travas de maneira segura, criando situações propensas a bloqueios e 
impasses (\english{deadlocks}). A componibilidade de travas seria necessária quando houver 
...
\section{O projeto Akka}

O projeto Akka \cite{akka} é composto por um conjunto de módulos escritos em 
...
e tolerantes a falhas. Suas principais características são: uma biblioteca de atores 
locais e remotos, suporte a STM, hierarquias de supervisão e uma combinação entre atores 
e STM (\english{``transactors''}) que da suporte a fluxos de mensagens baseados em eventos 
transacionais, assíncronos e componíveis. Akka oferece, ainda, uma série de módulos
...
e que unifica os paradigmas de programação funcional e orientado a objetos. Vem sendo desenvolvida desde 
...
\begin{itemize}
	...
	como uma classe abstrata, porém não definem construtores.  \english{Traits} são
	...
	única de uma classe. \english{Singleton objects} são definidos com a palavra-chave
...
Optamos por desenvolver este trabalho em Scala pelos fatores a seguir:

\begin{itemize}
	\item A linguagem é compilável para \english{bytecode} e pode ser executada
	na JVM;
	
	\item A linguagem interopera naturalmente com bibliotecas Java;
	
	\item Além das abstrações do paradigma de orientação a objetos, a linguagem implementa 
	também abstrações do paradigma funcional. Essas abstrações
	auxiliam na construção de um código mais enxuto e com menos verbosidade;
	
	\item O projeto Akka é primariamente escrito em Scala;
	
	\item A linguagem ter sido criada objetivando um melhor suporte para o
...
baseado na implementação do modelo de atores do projeto Akka. A idéia é substituir o mecanismo 
atualmente usado pelo Akka por um \english{message broker} AMQP. 
...
%% fazer em formato de lista
No Capítulo \ref{cap:atores}, apresentamos o modelo de atores, sua semântica e algumas
das implementações do modelo. No Capítulo \ref{cap:atores_akka} analisamos uma 
implementação do modelo de atores com mais detalhes
