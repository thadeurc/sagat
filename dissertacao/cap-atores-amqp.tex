...
\begin{itemize}
	\item \code{AMQPRemoteServer}: É uma classe utilizada no lado do servidor onde
	estão os atores remotamente acessíveis. Essa classe implementa os métodos abstrados
	de um \code{MessageHandler} e é responsável pelo recebimento,
	...
	remetentes, seja como consequência de envios feitos via \code{!!} e \code{!!!}, 
	...
	relacionamento bidirecional um para um entre um \code{AMQPRemoteServer} e uma ponte 
	servidora;
			
	\item \code{AMQPRemoteServerModule}: É uma feição que estende 
	a feição \code{RemoteServerModule} e define um nó onde atores
	...
	bidirecional um para um com um \code{AMQPRemoteServer}. As mensagens recebidas pelo 
	\code{AMQPRemoteServer} são encaminhadas para os atores residentes neste nó;

	\item \code{AMQPRemoteClient}:	É uma classe utilizada no lado do cliente
	...
	resultados futuros. Tal como a classe \code{AMQPRemoteServer}, essa classe também 
	implementa os métodos abstratos de um \code{MessageHandler} e é também é responsável
	...
	Existe uma relação bidirecional um para um entre um \code{AMQPRemoteClient}
	...
	\item \code{AMQPRemoteClientModule}: É uma feição que estende
	a feição \code{RemoteClientModule}. Essa feição
	que tem um papel complementar a \code{AMQPRemoteServerModule}. Seu
	papel principal é definir uma interface para envios de mensagens a atores
	remotos via \code{AMQPRemoteClient}. Ademais, esta feição feição
	mantém ainda um registro dos \code{AMQPRemoteClient} que são utilizados
	...
	\item \code{AMQPRemoteSupport}: É a implementação que tornamos acessível 
	via \code{Actor.remote}. Essa classe é responsável por concentrar as 
...
o nome do nó com base nos valores informados. O padrão utilizado é \code{host@porta}. 
Como mencionado seção \ref{sec:atores_locais}, \code{ActorRef}s
...
desse atributo (\code{homeAddress}) não é utilizado.
...
\code{Actor.remote} seja uma instância de \code{AMQPRemoteSupport}.
Ademais, devemos informar as configurações que nosso suporte espera, como por exemplo,
as políticas de armazenamendo e compartilhamento de canais. As alterações necessárias
...
\code{akka.conf}, optamos por adicionar novas propriedades para a configuração do 
...
\code{AMQPRemoteClient}s (e consequentemente de \code{ClientAMQPBridge}s). 
...
Pelo fato arquivo \code{akka.conf} ser único por JVM, a maneira pelo qual
...
caso onde dois clientes remotos seriam criados em uma JVM para
servidores remotos cujos nomes são \code{node1} e \code{node2}.
Os objetos que seriam criados pelos primeiro cliente remoto teriam o sufixo
\code{client.node1.<id>} e \code{client.node2.<id>}.	
Um outro detalhe importante é que, pelo fato de classe 
\code{AMQPRemoteClientModule} manter um registro com os clientes remotos, 
não há a possibilidade de, na mesma JVM, ser criado mais de uma instância de 
\code{AMQPRemoteClient} para um mesmo servidor remoto.
...
\begin{enumerate}
	...
	é desligado, o problema se limita ao retorno de eventuais mensagens que 
	ainda estavam na fila;
	
	\item Cenário oposto ao anterior, onde o servidor remoto possui uma configuração
...
O \english{cookie} de segurança é definido no arquivo \code{akka.conf} na seção \code{remote}. O uso de 
...
alterações no objeto \code{RemoteActorSerialization}. Esse objeto é o responsável por 
fabricar uma instância com as informações a serem seriadas. O método 
\code{createRemoteMessageProtocolBuilder} foi sobrecarregado e passou
...
implementação de \code{RemoteSupport} (que na figura \ref{fig:remote-actor-message-flow-amqp} é 
uma instância de \code{AMQPRemoteSupport}) para enviar a mensagem ao \code{RemoteSupport} 
que está no lado do servidor.

Os passos $3$ (transporte da mensagem do \code{ClientBootstrap} para
o \code{ServerBootstrap}) e $4$ (recebimento da mensagem pelo \code{ServerBootstrap} e 
...
e no repasse para a implementação de \code{MessageHandler} utilizada na ponte servidora.
...
Assim como na implementação com Netty, os envios feitos via métodos \code{!!} e 
\code{!!!}, também possuem um passo a mais do que os mostrados na figura 
...
As mensagem de resposta para envios feitos via \code{!!} e \code{!!!}, ou para notificar 
...
envio com \code{!!} ou \code{!!!}, o conteúdo da mensagem
