...
chamado de \english{Trading System}. Os desenvolvedores criaram também algumas classes que cuidam do armazenamento, comparação e geração de relatórios dos dados coletados em testes de desempenho.
...
de mensagens enviadas por todos os atores cliente foi sempre de $30000$. 

%x explicar como foram feitas as medidas
...
\hline
{Cenário} & {Clientes} & {TPS} & {média ($\mu s$)} & {25\% ($\mu s$)} & {50\% ($\mu s$)} & {75\% ($\mu s$)} & 
