...
\begin{itemize}
	...
	responde às mensagens vindas dos atores clientes com os endereços (referências) dos \english{order receivers}
	que foram criados.

	\item \english{Order}: É um componente que não é mostrado na figura, mas que representa
	uma ordem. É definida em uma classe abstrata e possui como informações o código da ação, o volume e 
...
O sistem possui ainda atores rotulados como ``clientes''. Um ator cliente é um ator externo ao 
...
é o \english{order receiver} para onde as mensagens serão enviadas.
...
é quase metade ($-47.50\%$) e o valor médio para cada envio/resposta leva quase o dobro do tempo ($90.33\%$).
Como consequência, a duração total também leva quase o dobro do tempo ($90.33$\%). Se observarmos os percentis, 
podemos notar que a diferença de tempo nas execuções com AMQP que estão abaixo da mediana, 
foram maior que duas vezes mais lentas em relação às execuções feitas com Netty ($140.69$\% e  $134.54$\%). 
Entretanto, quando comparamos os percentis acima da mediana, observamos uma boa redução na diferença 
($79.73$\% e  $39.31$\%). Em termos práticos, os resultados obtidos para $1$ ator cliente nos mostram
que $95$\% das ordens enviadas pelo ator cliente possuem como limite superior
de tempo de processamento $3260\mu s$ com Netty e $4550\mu s$ com AMQP, uma diferença de $39.31$\%,
bem abaixo do dobro do tempo, como esperávamos inicialmente.
...
atores clientes executaram diversos envios em paralelo para um \lstinline{RemoteClient} compartilhando, 
...
rede é um dos principais fatores que impactam nos resultados. A tabela \ref{tab:medidas-lentas} apresenta 
as melhores medidas obtidas em uma rede \english{wireless} com maior latência. Utilizamos a mesma 
...
Nosso argumento se baseia na comparação dos valores de ER/S obtidos para instâncias com $1$ e $2$ 
clientes mostrados na tabela \ref{tab:medidas}, com os valores de ER/S obtidos para instâncias com $10$
...
temos $501$ ER/S. Essa quantidade de ER/S só é obtida na rede de maior latência,
quando passamos de $8$ para $10$ atores clientes, onde os valores mostrados na tabela
\ref{tab:medidas-lentas} são respectivamente $491$ e $559$ ER/S. Quando passamos para $2$ atores
clientes, temos $894$ ER/S na tabela \ref{tab:medidas}. Essa quantidade de ER/S
só é aproximada na rede com maior latência quando utilizamos $80$ clientes. Nesse caso temos $889$ ER/S.

Analogamente, se compararmos o valor da tabela \ref{tab:medidas} para experimentos com AMQP e $1$ ator cliente, 
temos $263$ ER/S. Essa quantidade de ER/S só é aproximada na rede de maior latência,
quando utilizamos $10$ atores clientes, onde o valor mostrado na tabela
\ref{tab:medidas-lentas} é de $243$ ER/S. Quando passamos para $2$ atores
clientes temos $528$ como valor de ER/S na tabela \ref{tab:medidas}. Essa quantidade de ER/S
só é obtida na rede com maior latência quando utilizamos $80$ clientes. Nesse caso temos $628$
ER/S.

Com base na comparação da quantidade de ER/S, nossa conclusão é de que a quantidade de atores clientes
