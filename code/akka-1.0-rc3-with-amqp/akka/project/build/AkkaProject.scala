...
class AkkaParentProject(info: ProjectInfo) extends DefaultProject(info) with IdeaProject {
  ...
  // AMQP changes
  lazy val rabbit = ModuleConfiguration("com.rabbitmq", sbt.DefaultMavenRepository)
  ...
  lazy val akka_remote      = project("akka-remote",      "akka-remote",      new AkkaRemoteProject(_) with IdeaProject,    akka_typed_actor)
...
//  override def runClasspath = super.runClasspath +++ "config"
  ...
  class AkkaRemoteProject(info: ProjectInfo) extends AkkaDefaultProject(info, distPath) with IdeaProject{
    ...
    val rabbitmq      = Dependencies.rabbit
