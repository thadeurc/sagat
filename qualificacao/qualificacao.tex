...
\newpage
\section{AMQP e o Tráfego de Mensagens entre Atores Remotos}
...
\subsection{AMQP}
	...
	a mensagem possam estar pervasivamente disponíveis e nas redes das empresas, tornando possível o desenvolvimento de novas aplicações
	\cite{amqp-spec}.%% to do - melhorar referencia?
	
	\par O modelo AMQP contempla a definição explícita da semântica do servidor, já que este é um fator chave para interoperabilidade.
	A seguir a descrição dos principais componentes: 
	\begin{itemize}
		...
		até que elas sejam enviadas em sequência para as aplicações consumidoras. As filas são entidades de armazendamento e
		...
		para as filas de mensagens, levando em conta critérios pré-definidos (\english{bindings}). \english{Exchanges} inspecionam
		as mensagens checando, na maioria dos casos pela chave de roteamento, e com o auxílio de uma tabela de \english{bindings}, 
		decidem como encaminhar as mensagens às respectivas filas, jamais fazendo o armazenamento das mensagens;		
		...
		\item \english{Virtual hosts}: São coleções de \english{exchanges}, filas de mensagens e objetos associados. \english{Virtual hosts}
	...
	uma determinada \english{exchange} de um determinado \english{virtual host}.Uma vez que a mensagem tenha chegado no servidor
	...
	roteamento para uma \english{exchange} alterativa. A escolha depende do comportamento escolhido pelo produtor. Quando a mensagem
