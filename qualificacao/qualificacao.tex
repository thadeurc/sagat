...
\subsection{Atores em Scala}
	...
	padrões $padrao_i$, é removida da caixa de correio e a ação $acao_i$ correspondente é executada. Caso não haja casamento
	de nenhum dos padrões, o ator é suspendido.

	\par Existem algumas recomendações para se atingir uma boa performance
	...
	interfaces remotas como em Java RMI. O trafégo das mensagens é feito via serialização padrão Java através de \english{sockets}
	 TCP.
...
da mensagem para o \english{RemoteServer} associado. Uma vez que a mensagem tenha sido recebida, o \english{Handler} faz a análise
...
atores remotos pelo projeto AKKA, baseado em \english{sockets}, para AMQP. O motivo pelo qual fizemos a opção pela
implementação do projeto AKKA ao invés da padrão existente na distribuição de Scala, deve-se ao fato de, mesmo ainda
em desenvolvimento, esta implementação se mostra mais ampla e mais robusta, sendo no nosso entendimento passível de 
se tornar parte da distribuição da linguagem Scala no futuro.

\subsection{Visão geral da implementação}
\par A implementação da troca do mecanismo de transporte entre as mensagens se dará em duas partes:
\begin{enumerate}
	\item Substituição do JBoss Netty pela estrutura AMQP: Iremos retirar todos os componentes relacionados e
	adicionaremos no seus lugares a estrutura necessária para o trafego das mensagens. Tentaremos presentes
	as características que tiverem relevância, possivelmente os diversos formatos de mensagens e a compactação;
	
	\item Alinhamento do recebimento e envio de mensagens via AMQP nas transações existentes: Iremos adicionar
	o envio e recebimento de mensagens transacional provido pela específicação AMQP como parte do controle 
	transactional feito pelo AKKA através de STM.
\end{enumerate}

\par A implementação será feita utilizando como \english{broker} AMQP o projeto RabbitMQ\cite{rabbitmq}. A criação
de componentes se dará da seguinte maneira:\entry{(i)} Uma \english{exchange} será criada para cada par 
\english{RemoteServer/RemoteClient}; \entry{(ii)} Um par de filas será criado por \english{exchange} para 
armazenar as mensagens \english{RemoteClient-RemoteServer} e uma para armazenar as mensagens 
\english{RemoteServer-RemoteClient}. A figura~\ref{} apresenta uma visão geral do modelo a ser implementado.
